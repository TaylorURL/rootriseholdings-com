...
/** Validate the login form. Returns a map of field → error message. */
...
/** DESIGN-ONLY sign-in page. Wired to the stubbed auth context, no real backend. */
export default function LoginPage() {
  ...
  return (
    <AuthShell
      ...
      title="Sign in to your terminal"
      subtitle="Pick up where the signals left off. Your watchlists and alerts are waiting."
    >
      ...
      <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-5">
        ...
        <div className="flex flex-col gap-1.5">
          <div className="flex items-center justify-between">
            ...
            <a
              href="#"
              className="text-xs font-medium text-accent-bright transition-colors hover:text-accent"
            >
              Forgot password?
            </a>
          </div>
...
/** Provider sign-in placeholders. Visually complete; no-op handlers. */
function OAuthButtons({ onClick, disabled }) {
  return (
    <div className="grid grid-cols-2 gap-3">
      <OAuthButton onClick={onClick} disabled={disabled} label="Continue with Google">
        <GoogleMark />
        Google
      </OAuthButton>
      <OAuthButton onClick={onClick} disabled={disabled} label="Continue with Apple">
        <AppleMark />
        Apple
      </OAuthButton>
    </div>
