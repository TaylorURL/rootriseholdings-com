  ...
  return (
    <Section id="features" tone={tone} className="border-t border-border">
      <Container>
        ...
        <StaggerGroup
          className="card-elevated mt-16 grid grid-cols-1 gap-px overflow-hidden rounded-2xl border border-border bg-border sm:grid-cols-2 lg:grid-cols-3"
          stagger={0.06}
        >
          {PLATFORM_FEATURES.map((feature) => (
            <StaggerItem key={feature.title} className="h-full">
              <SpotlightCard className="h-full bg-bg p-8 transition-[background-color,border-color] duration-300 [transition-timing-function:var(--ds-ease-out)] [@media(hover:hover)]:hover:bg-surface/40">
                <span className="flex h-11 w-11 items-center justify-center rounded-xl border border-border bg-surface-2 text-accent-bright transition-[transform,border-color,color] duration-300 [transition-timing-function:var(--ds-ease-out)] [@media(hover:hover)]:group-hover:-translate-y-0.5 [@media(hover:hover)]:group-hover:border-border-hover">
